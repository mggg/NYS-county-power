...
def main(
    ...
    file_name = f"Ontario_MCMCVETO_discrep_L1_lenburst_{burst_length}_iters_{n_bursts}_pop_{population}_T_{threshold}_id_{uuid}_dyn_fuzz_{random_fuzz_epsilon}.jsonl"
        ...
        for i in tqdm(
            ...
            for j in range(1, burst_length+1):
                ...
                if discrep_temp < best_local_discrep:
                    ...
                    best_local_u = cu_u_temp
                    best_local_power = power_temp
                    best_substep = j
            ...
            if best_local_discrep < best_so_far:
                ...
                assert not any(best_u >= best_u.sum()*(1-threshold)), "BAD best_u allowed"
                print(
                    (
                        f"{str('{')}\"step\": {i}, "
            ...
            if i - previous_write_idx > 100 and i - previous_burst_idx > 100:
